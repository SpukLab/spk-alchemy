...
function applyGate(samples, channels, sampleRate, thresholdRms) {
  ...
  const windowFrames2 = Math.max(1, Math.round(sampleRate * GATE_WINDOW_MS / 1e3));
  const windowCount = Math.max(1, Math.ceil(totalFrames / windowFrames2));
  const targets = new Float64Array(windowCount);
  for (let w = 0; w < windowCount; w++) {
    const start = w * windowFrames2;
    const end = Math.min(totalFrames, start + windowFrames2);
    let sumSquares = 0;
    let count = 0;
  ...
  const out = new Int16Array(samples.length);
  const centerOf = (w) => w * windowFrames2 + windowFrames2 / 2;
  for (let f = 0; f < totalFrames; f++) {
    const w = Math.min(windowCount - 1, Math.floor(f / windowFrames2));
...
function detectLocalEvents(samples, channels, windowFrames2) {
  const totalFrames = channels > 0 ? samples.length / channels : 0;
  if (totalFrames === 0 || windowFrames2 <= 0) return [];
  const windowCount = Math.max(1, Math.ceil(totalFrames / windowFrames2));
  ...
  for (let w = 0; w < windowCount; w++) {
    ...
    if (rise > RISE_THRESHOLD && energies[w] > FLOOR) {
      candidates.push({ frame: w * windowFrames2, strength: rise });
    }
  }
  candidates.sort((a, b) => b.strength - a.strength || a.frame - b.frame);
  return candidates;
}
function mixAdd(base, addition, atFrame, channels, gainNumerator, gainDenominator) {
...
function localGainBoost(samples, channels, atFrame, lengthFrames, gainNumerator, gainDenominator, fadeFrames) {
  const totalFrames = channels > 0 ? samples.length / channels : 0;
...
var WINDOW_MS = 12;
function candidateWindowFrames(sampleRate) {
  return Math.max(4, Math.round(sampleRate * WINDOW_MS / 1e3));
}
function deterministicShuffle(items, rng) {
  const out = [...items];
  for (let i = out.length - 1; i > 0; i--) {
    const j = Math.floor(rng() * (i + 1));
    [out[i], out[j]] = [out[j], out[i]];
...
var ACENTOS_DENSITY_CEILING_FRACTION = 0.5;
function acentosSelectionCount(presencia, candidateCount) {
...
function applyAcentos(samples, channels, sampleRate, presencia, seleccion, seed) {
  ...
  const fadeFrames = Math.max(1, Math.round(sampleRate * 4 / 1e3));
  const lengthFrames = Math.max(fadeFrames * 2, candidateWindowFrames(sampleRate) * 2);
  const gainNumerator = 10 + Math.round(Math.max(0, Math.min(100, presencia)) / 100 * 6);
  let out = samples;
...
var GOTEROS_MAX_DROPLETS = 24;
function goterosDropletCount(cantidad) {
...
function applyGoteros(source, output, channels, sampleRate, cantidad, variacion, seed) {
  ...
  const dropletMs = 15 + v / 100 * 45;
  const dropletFrames = Math.max(4, Math.min(
    Math.floor(sourceFrames / 4),
    Math.round(sampleRate * dropletMs / 1e3)
  ));
  const fadeFrames = Math.max(1, Math.round(sampleRate * 3 / 1e3));
  const rng = seededRandom(seed);
  ...
  for (let i = 0; i < count; i++) {
    ...
    let droplet = source.samples.slice(
      regionStart * source.channels,
      (regionStart + dropletFrames) * source.channels
    );
    ...
    out = mixAdd(out, droplet, atFrame, channels, 8, 10);
  }
  return out;
}
function applyEventos(source, transformed, channels, sampleRate, params, seed) {
...
function validateMesaState(state) {
  ...
  return {
    ...
    goteros: { cantidad: clamp01to1002(goteros.cantidad), variacion: clamp01to1002(goteros.variacion) },
    acentos: { presencia: clamp01to1002(acentos.presencia), seleccion: clamp01to1002(acentos.seleccion) }
  };
}
...
var MEDIUM_STRATEGIES = [
  {
    ...
    eventWeights: { goteros: 0.3, acentos: 0.8 },
    // few Goteros; meaningful Acentos
    anchor: null,
  ...
  },
  {
    ...
    eventWeights: { goteros: 0.6, acentos: 0.6 },
    // moderate Goteros derived from slices; moderate accents
    anchor: null,
    dominantTreatment: "balanced"
  },
  {
    ...
    eventWeights: { goteros: 0.25, acentos: 0.7 },
    // sparse Goteros; accent timing interacts with temporal scaling
    anchor: null,
    dominantTreatment: "balanced"
  },
  {
    ...
    eventWeights: { goteros: 0.5, acentos: 0.35 },
    // microscopic droplets; subtle accents
    anchor: null,
    dominantTreatment: "balanced"
  }
];
var UNEXPECTED_STRATEGIES = [
  {
    ...
    eventWeights: { goteros: 0.9, acentos: 0.6 },
    // irregular temporal droplets; displaced accents
  ...
  },
  {
    ...
    eventWeights: { goteros: 1.3, acentos: 0.8 },
    // dense micro-droplets; repeated tiny accent structures
  ...
  },
  {
    ...
    eventWeights: { goteros: 0.5, acentos: 1.4 },
    // fewer but stronger accented events; locally excited droplets
    anchor: "onset",
  ...
  },
  {
    ...
    eventWeights: { goteros: 0.9, acentos: 0.9 },
    // strongest interaction; still bounded, not maximum everything
...
function translateEventos(state, strategy) {
...
var RELATIONAL_CONFIGURATION_ID = "mesa-relational-v1";
var RELATIONAL_VERSION = "1.0.0";
var clamp01to1003 = (v) => Math.max(0, Math.min(100, Math.round(v)));
var RELATION_MODES = ["solo", "inyectar", "acentuar-con", "contagiar", "cruzar"];
function validateRelationalContext(ctx) {
  ...
  return {
    ...
    relationMode,
    guestInfluence: clamp01to1003(ctx.guestInfluence)
  };
}
function adaptChannelCount(samples, fromChannels, toChannels) {
  ...
  for (let f = 0; f < frames; f++) {
    if (toChannels > fromChannels) {
      ...
      for (let c = 0; c < toChannels; c++) {
        let sum = 0, count = 0;
        for (let s = c; s < fromChannels; s += toChannels) {
          sum += samples[f * fromChannels + s];
          count += 1;
        }
...
function adaptGuestToSource(guest, source) {
...
var WINDOW_MS2 = 12;
function windowFrames(sampleRate) {
  return Math.max(4, Math.round(sampleRate * WINDOW_MS2 / 1e3));
}
function guestFragment(guest, regionStart, lengthFrames, channels, fadeFrames) {
  ...
  return applyBoundaryFade(
    guest.samples.slice(start * channels, (start + len) * channels),
    channels,
    fadeFrames,
    fadeFrames
  );
}
function inyectar(sourceSamples, channels, sampleRate, guest, influence, seed) {
  ...
  const fragFrames = Math.max(4, Math.min(Math.floor(guestFrames / 4), Math.round(sampleRate * fragMs / 1e3)));
  const fadeFrames = Math.max(1, Math.round(sampleRate * 3 / 1e3));
  const spacing = outputFrames / count;
...
function acentuarCon(sourceSamples, channels, sampleRate, guest, influence, seed) {
  ...
  if (count === 0) return sourceSamples;
  const chosen = sourceCandidates.slice(0, count);
  ...
  chosen.forEach((c, i) => {
    ...
    const gainNumerator = 6 + Math.round(influence / 100 * 6);
    out = mixAdd(out, frag, c.frame, channels, gainNumerator, 10);
  });
  return out;
}
function contagiar(sourceSamples, channels, sampleRate, guest, influence, seed) {
  const outputFrames = channels > 0 ? sourceSamples.length / channels : 0;
  const guestFrames = channels > 0 ? guest.samples.length / channels : 0;
  if (outputFrames === 0 || guestFrames === 0) return sourceSamples;
  const count = Math.max(0, Math.min(36, Math.round(influence / 100 * 36)));
  if (count === 0) return sourceSamples;
  const rng = seededRandom(seed);
  const microMs = 6 + influence / 100 * 10;
  const microFrames = Math.max(3, Math.min(Math.floor(guestFrames / 4), Math.round(sampleRate * microMs / 1e3)));
  const fadeFrames = Math.max(1, Math.round(sampleRate * 2 / 1e3));
  const spacing = outputFrames / count;
  const gainNumerator = 3 + Math.round(influence / 100 * 3);
  let out = sourceSamples;
  for (let i = 0; i < count; i++) {
...
function cruzar(sourceSamples, channels, sampleRate, guest, influence, seed) {
...
function applyRelation(sourceSamples, channels, sampleRate, guest, mode, influence, seed) {
  switch (mode) {
    case "inyectar":
      return inyectar(sourceSamples, channels, sampleRate, guest, influence, seed);
    case "acentuar-con":
      return acentuarCon(sourceSamples, channels, sampleRate, guest, influence, seed);
    case "contagiar":
      return contagiar(sourceSamples, channels, sampleRate, guest, influence, seed);
    case "cruzar":
      return cruzar(sourceSamples, channels, sampleRate, guest, influence, seed);
    case "solo":
    default:
      return sourceSamples;
  }
}
function runRelationalMesaExploration(sourceBytes, guestBytes, mesaState, baseSeed, ctx) {
  ...
  if (!guestBytes || validated.relationMode === "solo") {
    return sourceObservations.map((o) => ({ ...o, relationMode: "solo", guestContributed: false }));
  }
  ...
  return sourceObservations.map((obs) => {
    const relationSeed = obs.seed + 777001 >>> 0;
    ...
    let samples = applyRelation(
      ...
      rendered.sampleRate,
      guestCanonical,
      validated.relationMode,
      validated.guestInfluence,
      relationSeed
    );
    const level = measurePreviewLevel(samples);
    if (level.peak > PREVIEW_PEAK_CEILING && level.peak > 0) {
    ...
    return {
      ...obs,
      bytes: encodeWav({ ...rendered, samples }),
      relationMode: validated.relationMode,
...
var AlchemyService = class {
  ...
  async runRelationalMesaExploration(input) {
    ...
    const relationalCtx = validateRelationalContext({
      ...
      guestMaterialId: input.guestMaterialId,
      relationMode: input.relationMode,
      guestInfluence: input.guestInfluence
    });
    const mesaState = validateMesaState(input.mesaState);
    const relationalConfigStub = {
      ...
      implementationVersion: MESA_VERSION,
      operations: [{
        operation: "relational-mesa-exploration",
    ...
    const experiment = await this.createExperiment({
      ...
      operation: "exploration",
      parameters: { mesaState, relationalCtx },
      agentId: input.agentId,
      configuration: relationalConfigStub,
      baseSeed: input.baseSeed
    });
    const observations = runRelationalMesaExploration(
      sourceBytes,
      guestBytes,
      mesaState,
      input.baseSeed,
    ...
    const variations = observations.map((obs, index) => {
      ...
      const preview = {
        ...
        parameters: {
          ...
          conditioningResolved: resolveConditioningParameters(conditioningState),
          relational: {
            configurationId: RELATIONAL_CONFIGURATION_ID,
            configurationVersion: RELATIONAL_VERSION,
            sourceMaterialId: source.id,
            guestMaterialId: guest?.id ?? null,
            relationMode: obs.relationMode,
            guestInfluence: relationalCtx.guestInfluence,
    ...
    return {
      ...
      createdAt: this.#clock(),
      mesaState,
      relationalContext: relationalCtx
    };
  }
  // ---- helpers -------------------------------------------------------------
...
async function immediateParents(materialId, queries) {
...
var AlchemicalIdentityRegistry = class {
  #inner;
  constructor(store, paletteSize = LINEAGE_PALETTE.length) {
  ...
  async colorForIdentity(materialId) {
...
async function ancestryMarkers(materialId, queries, lineageRegistry, identityRegistry) {
  const parentIds = await immediateParents(materialId, queries);
  const markers = [];
  for (const parentId of parentIds) {
    const color = await identityRegistry.hasIdentity(parentId) ? await identityRegistry.colorForIdentity(parentId) : await lineageRegistry.colorForMaterial(parentId, queries);
...
var IDENTITY_REGISTRY_KEY = "alchemy.identity-palette.v1";
var LocalStorageLineageStore = class {
  #key;
  constructor(key = LINEAGE_REGISTRY_KEY) {
    this.#key = key;
  }
  async read() {
    try {
  ...
  async write(assignments) {
    try {
      globalThis.localStorage?.setItem(this.#key, JSON.stringify(assignments));
    } catch {
    }
  }
};
async function openWebLab() {
  ...
  return {
    ...
    async guestCandidates(excludeMaterialId) {
      const page = await queries.promotedMaterials(void 0, 100);
      return [...page.items].reverse().filter((m) => m.id !== excludeMaterialId);
    },
    async exploreRelationalMesa(sourceMaterialId, guestMaterialId, question, state, relationMode, guestInfluence, conditioning) {
    ...
    async identityColorIfAny(materialId) {
      return await identityRegistry.hasIdentity(materialId) ? identityRegistry.colorForIdentity(materialId) : null;
    },
